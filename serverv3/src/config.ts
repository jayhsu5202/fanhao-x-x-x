...
export const config = {
  ...
  /** 同時執行的下載工作數（每個工作一個 ffmpeg 子程序）；見 `parseConcurrencyEnv` */
  downloadQueueConcurrency: parseConcurrencyEnv(process.env.DOWNLOAD_QUEUE_CONCURRENCY, 6, 1),
  /**
   * MissAV 文件抓取會重用長駐 session pool。
   */
  missavSessionPoolSize: Math.max(
    1,
    Math.min(16, Number.parseInt(process.env.MISSAV_SESSION_POOL_SIZE || "4", 10) || 4)
  ),
  missavFetchTimeoutMs: Math.min(
    60_000,
    Math.max(5_000, Number.parseInt(process.env.MISSAV_FETCH_TIMEOUT_MS || "22000", 10) || 22_000)
  ),
  missavFetchRetries: Math.min(
    6,
    Math.max(1, Number.parseInt(process.env.MISSAV_FETCH_RETRIES || "4", 10) || 4)
  ),
