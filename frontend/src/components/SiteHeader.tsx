      ...
      {open ? (
        <div className="site-nav-panel" id={menuId} role="dialog" aria-label="導覽與分類">
          <div className="site-nav-panel-inner">
            ...
            {NAV_MENU.map((m) => (
              <details key={m.key} className="site-nav-accordion">
                <summary className="site-nav-accordion-summary">{m.label}</summary>
                <div className="site-nav-accordion-body">
                  <Link className="site-nav-panel-sublink site-nav-panel-sublink-strong" to={m.to} onClick={() => setOpen(false)}>
                    進入{m.label}
                  </Link>
                  {m.children.map((ch) => (
                    <Link
